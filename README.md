...
| Assistant | `A` or the sparkle button — describe a board and it builds it, ask for a change and it edits the board, press Fix on a check finding or "Fix checks" and it resolves them, "Fill in details" fills part numbers from presets. Bring your own key: Claude by default, plus OpenRouter, Z.AI GLM, Moonshot Kimi, any OpenAI-compatible endpoint, and local Ollama, which also runs Ollama Cloud models by their :cloud tag. Each reply is one undo step and what it touched glows until your next click. The key stays in this browser (only if you tick remember) and is never part of the board, autosave, or share links |
