...
test('a preflight is answered with the CORS headers the browser asked for', async () => {
  const r = await handle(req('/ollama.com/api/chat', { method: 'OPTIONS', headers: { 'access-control-request-method': 'POST', 'access-control-request-headers': 'authorization,content-type' } }));
...
test('a POST is forwarded to the named upstream with only the auth, content-type, and accept headers', async () => {
  const calls = [];
  const r = await handle(req('/ollama.com/api/chat?x=1', {
    method: 'POST',
  ...
  assert.equal(calls[0].url, 'https://ollama.com/api/chat?x=1');
...
test('the upstream status and a streaming body pass through unchanged', async () => {
  ...
  const r = await handle(req('/api.moonshot.ai/v1/chat/completions', { method: 'POST', body: '{}' }), {}, async (url, init) => { calls.push(url); return new Response(stream, { status: 429, headers: { 'content-type': 'application/x-ndjson', 'retry-after': '3' } }); });
...
test('unknown upstreams, other methods, and the root are answered by the relay itself, with CORS', async () => {
  ...
  const del = await handle(req('/ollama.com/api/tags', { method: 'DELETE' }), {}, fakeUpstream(calls));
...
test('UPSTREAMS and ORIGINS narrow what the relay serves', async () => {
  ...
  const other = await handle(req('/ollama.com/api/tags'), env, fakeUpstream(calls));
  assert.equal(other.status, 403, 'a browser origin off the list is refused');
  assert.equal(other.headers.get('access-control-allow-origin'), null);
  const ok = await handle(new Request('https://relay.example/ollama.com/api/tags', { headers: { origin: 'https://allowed.example' } }), env, fakeUpstream(calls));
  assert.equal(ok.status, 200);
  assert.equal(ok.headers.get('access-control-allow-origin'), 'https://allowed.example');
  const curl = await handle(new Request('https://relay.example/ollama.com/api/tags'), env, fakeUpstream(calls));
...
test('an upstream that cannot be reached is reported as 502 from the relay', async () => {
  const r = await handle(req('/ollama.com/api/chat', { method: 'POST', body: '{}' }), {}, async () => { throw new Error('boom'); });
